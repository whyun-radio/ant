...
- 经典结构：
    ...
    ![](https://www.w8ji.com/Baluns/sleeve1.gif)
...
为了绕制方便，我们选择了 PVC 水管。一般接收 DTMB 用的馈线直径在 7mm 作用，使用 PVC 管绕制的话，得需要很粗的管子才行，所以我们这里选择了细铜线。加之我们制作的是 1：1 巴伦，所以不需要考虑导线的特性阻抗，只需要考虑 1.1 和 1.2 小节提到的平衡不平衡转化和共模电流抑制的问题。

由于并绕双线产生的磁场相互耦合解决了平衡转不平衡的问题。同时我们将双线绕制 4-6 圈后，产生的 `L共模​` 足够大，可以使 `公式 1.2.2 `产生的共模电阻足够大。

> 这得益于 DTMB 使用的 470-700Mhz 频段，使公式中的 `L共模​` 只需要值不需要特别大，所以绕制 4-6 圈即可。
...
2. 取两根铁氟龙导线平行紧密绕制在 PVC 管上绕制 4-5 圈，然后用胶带将其固定在 PVC 管上。
3. 在巴伦一侧接上接线端子母头，另一侧接上 F 头。**注意两侧的接线头要尽量多，以不超过 5cm 为宜**。
