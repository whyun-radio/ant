...
为了验证天线的实际效果，我选择了短波 15340 kHz 频点进行对比测试。测试是在室内进行的，周围墙体和家用电器都会对短波接收造成一定影响，所以结果只能作为一个简单参考。测试时先不接外置天线，仅使用收音机自身的接收能力，此时屏幕显示信号强度大约为 26 dBμ。

![](images/without-ant.jpg)

图 4.1 未接外置天线时的接收效果

接上制作好的无源屏蔽环天线后，在相同频点下信号强度提升到大约 43 dBμ，提升幅度比较明显。实际听感上，声音也更加稳定，背景噪声有所降低，弱信号更容易从噪声中分辨出来。

![](images/with-ant.jpg)

图 4.2 接上无源屏蔽环天线后的接收效果

从这次简单对比来看，这个天线在短波接收上还是有一定改善效果的。不过由于测试环境是在室内，天线没有完全展开到更理想的接收环境中，所以实际效果可能还没有达到它的预期表现。由于小环天线具有方向性，使用时可以慢慢旋转天线角度，寻找信号最强、噪声最低的位置；在室内使用时，也可以尽量远离开关电源、电脑、充电器等干扰源，这样效果会更明显。
